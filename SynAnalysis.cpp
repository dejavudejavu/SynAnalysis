...
int emptyBox[Max_Proc];//可推出空的非终结符的编号
...
extern vector<pair<const char*, int> > nameMap;
...
int seekCodeNum(char* word)
{
    ...
    {
        //处理标志符
        terMap.push_back(make_pair(word, IDENFR));
        return IDENFR;
    }
    //else if (strcmp(word, "string") == 0)
    //{
    //    //处理标志符
    //    terMap.push_back(make_pair(word, STRING));
    //    return STRING;
    //}
    else
    {
...
void initGrammer()
{
    ...
    memset(emptyBox, -1, sizeof(emptyBox));
...
void Analysis()
{
    ...
    while (p != NULL)
    {
        ...
        //if (p->type == AUTO || p->type == CONSTTK || p->type == UNSIGNED || p->type == SIGNED
        ...
        else if (p->type == CHARCON)
        {
            reserve[i++] = CHARACTER;
            //Push(&s2,DIGIT);
        }
        //else if (p->type == CHARTK || p->type == DOUBLE || p->type == FLOAT || p->type == INTTK ||
        //    p->type == LONG || p->type == SHORT || p->type == VOIDTK)
        //{
        //    reserve[i++] = TYPE;
        //    //Push(&s2,TYPE);
        //}
    ...
    for (i = 0;; i++)   /*分析*/
    {
        ...
        {
            resultfile << "所用推出式：";
            string p = searchMapping(M[h1][h2][0]);
            const char q = p.at(2);
            if (q < 0) {
                printf("%s\n", searchMapping(M[h1][h2][0]));
            }
            int w = 0;
            //记录下推导式的长度
            for (w = 0;; w++)
