...
//输入关键字和名称对
vector<pair<const char*, int> > nameMap;
...
void initKeyMapping()
{
    ...
    keyMap.push_back(make_pair("void", VOIDTK));
    keyMap.push_back(make_pair("volatile", VOLATILE));
    keyMap.push_back(make_pair("while", WHILETK));
    keyMap.push_back(make_pair("main", MAINTK));
    keyMap.push_back(make_pair("scanf", SCANFTK));
    keyMap.push_back(make_pair("printf", PRINTFTK));
    ...
    keyMap.push_back(make_pair("character", CHARACTER));
}
void initOperMapping()
{
    ...
    operMap.push_back(make_pair("*", MULT));
    operMap.push_back(make_pair("/", DIV));
    operMap.push_back(make_pair("%", MOD));
    operMap.push_back(make_pair("+", PLUS));
    operMap.push_back(make_pair("-", MINU));
    operMap.push_back(make_pair("<", LSS));
    operMap.push_back(make_pair(">", GRE));
    ...
    operMap.push_back(make_pair("<=", LEQ));
    operMap.push_back(make_pair(">=", GEQ));
    operMap.push_back(make_pair("==", EQL));
    operMap.push_back(make_pair("!=", NEQ));
    ...
    nameMap.push_back(make_pair("CHARTK", CHARTK));
    nameMap.push_back(make_pair("CONSTTK", CONSTTK));
    nameMap.push_back(make_pair("DOTK", DOTK));
    nameMap.push_back(make_pair("ELSETK", ELSETK));
    nameMap.push_back(make_pair("FORTK", FORTK));
    nameMap.push_back(make_pair("IFTK", IFTK));
    nameMap.push_back(make_pair("INTTK", INTTK));
    nameMap.push_back(make_pair("RETURNTK", RETURNTK));
    nameMap.push_back(make_pair("VOIDTK", VOIDTK));
    nameMap.push_back(make_pair("WHILETK", WHILETK));
    nameMap.push_back(make_pair("MAINTK", MAINTK));
    nameMap.push_back(make_pair("SCANFTK", SCANFTK));
    nameMap.push_back(make_pair("PRINTFTK", PRINTFTK));
    nameMap.push_back(make_pair("STRING", STRING));
    nameMap.push_back(make_pair("DIGIT", DIGIT));
    nameMap.push_back(make_pair("CHARACTER", CHARACTER));
    nameMap.push_back(make_pair("DIV", DIV));
    nameMap.push_back(make_pair("MULT", MULT));
    nameMap.push_back(make_pair("PLUS", PLUS));
    nameMap.push_back(make_pair("MINU", MINU));
    nameMap.push_back(make_pair("LSS", LSS));
    nameMap.push_back(make_pair("GRE", GRE));
    nameMap.push_back(make_pair("ASSIGN", ASSIGN));
    nameMap.push_back(make_pair("ASSIGN", ASSIGN));
    nameMap.push_back(make_pair("GEQ", GEQ));
    nameMap.push_back(make_pair("EQL", EQL));
    nameMap.push_back(make_pair("NEQ", NEQ));
    nameMap.push_back(make_pair("LPARENT", LPARENT));
    nameMap.push_back(make_pair("RPARENT", RPARENT));
    nameMap.push_back(make_pair("LBRACK", LBRACK));
    nameMap.push_back(make_pair("RBRACK", RBRACK));
    nameMap.push_back(make_pair("LBRACE", LBRACE));
    nameMap.push_back(make_pair("RBRACE", RBRACE));
    nameMap.push_back(make_pair("COMMA", COMMA));
    nameMap.push_back(make_pair("SEMICN", SEMICN));
}
void initLimitMapping()
{
    limitMap.clear();
    limitMap.push_back(make_pair("(", LPARENT));
    limitMap.push_back(make_pair(")", RPARENT));
    limitMap.push_back(make_pair("[", LBRACK));
    limitMap.push_back(make_pair("]", RBRACK));
    limitMap.push_back(make_pair("{", LBRACE));
    limitMap.push_back(make_pair("}", RBRACE));
    ...
    limitMap.push_back(make_pair(";", SEMICN));
    ...
    while (ch != EOF)
    {
        ...
        else if (ch >= '0' && ch <= '9')
        {
            ...
            else
            {
                if (flag2 == 0)
                {
                    createNewNode(word, CONSTANT_DESC, INTCON, -1, line);
                }
                else
                {
        ...
        else if (ch == '\'')
        {
            ...
            memcpy(word, array, i);
            word[i] = '\0';
            createNewNode(word, CONSTANT_DESC, CHARCON, -1, line);
            createNewNode("\'", CLE_OPE_DESC, SIN_QUE, -1, line);
        }
        ...
        else
        {
            ...
            else if (ch == '-')
            {
                ...
                {
                    createNewNode("-", OPE_DESC, MINU, -1, line);
            ...
            else if (ch == '+')
            {
                ...
                {
                    createNewNode("+", OPE_DESC, PLUS, -1, line);
                    fseek(infile, -1L, SEEK_CUR);
                }
            }
            //处理*开头的运算符
            else if (ch == '*')
            {
                ...
                {
                    createNewNode("*", OPE_DESC, MULT, -1, line);
                    fseek(infile, -1L, SEEK_CUR);
                }
            }
            //处理按^开头的运算符
            ...
            else if (ch == '!')
            {
                ...
                if (ch == '=')
                {
                    createNewNode("!=", OPE_DESC, NEQ, -1, line);
                }
                else
                {
            ...
            else if (ch == '<')
            {
                ...
                {
                    createNewNode("<=", OPE_DESC, LEQ, -1, line);
                }
                else
                {
                    createNewNode("<", OPE_DESC, LSS, -1, line);
                    fseek(infile, -1L, SEEK_CUR);
                }
            }
            //处理>开头的运算符
            else if (ch == '>')
            {
                ...
                {
                    createNewNode(">=", OPE_DESC, GEQ, -1, line);
                }
                else
                {
                    createNewNode(">", OPE_DESC, GRE, -1, line);
                    fseek(infile, -1L, SEEK_CUR);
                }
            }
            //处理|开头的运算符
            ...
            else if (ch == '=')
            {
                ...
                if (ch == '=')
                {
                    createNewNode("==", OPE_DESC, EQL, -1, line);
                }
                else
                {
                    createNewNode("=", OPE_DESC, ASSIGN, -1, line);
                    fseek(infile, -1L, SEEK_CUR);
                }
            }
            else if (ch == '(')
            {
                ...
                createNewNode("(", CLE_OPE_DESC, LPARENT, -1, line);
            }
            else if (ch == ')')
            {
                rightSmall++;
                lineBra[1][rightSmall] = line;
                createNewNode(")", CLE_OPE_DESC, RPARENT, -1, line);
            }
            else if (ch == '[')
            {
                leftMiddle++;
                lineBra[2][leftMiddle] = line;
                createNewNode("[", CLE_OPE_DESC, LBRACK, -1, line);
            }
            else if (ch == ']')
            {
                rightMiddle++;
                lineBra[3][rightMiddle] = line;
                createNewNode("]", CLE_OPE_DESC, RBRACK, -1, line);
            }
            else if (ch == '{')
            {
                leftBig++;
                lineBra[4][leftBig] = line;
                createNewNode("{", CLE_OPE_DESC, LBRACE, -1, line);
            }
            else if (ch == '}')
            {
                rightBig++;
                lineBra[5][rightBig] = line;
                createNewNode("}", CLE_OPE_DESC, RBRACE, -1, line);
            }
            else if (ch == '.')
            {
            ...
            {
                createNewNode(";", CLE_OPE_DESC, SEMICN, -1, line);
            }
            else
            {
                char temp[2];
